// ГАЙД ПО МИНИФИКАЦИИ - $id() это обычный док гет элембуид. innerMain() это заменить весь контект внутри div id=1st
 ...
 * после вызывается функция '$.link.get()', которая берёт в урл всё после '?' и прогоняя через себя вызывает нужные функции (например '?guides' закинет в гайды, или '?gdps=45' откроет гдпс с айди 45)
 * после выполнения '$.link.get()' хелпер готов к работе с клиентом
*/
...
helperCaptchaSiteKey = '6Ldrt0grAAAAAMdteG7pq6LZ1UYeMvkElvUV7Qhx',//'6Ldrt0grAAAAAMdteG7pq6LZ1UYeMvkElvUV7Qhx',
globalWiki = 0,
wikiTemplates = {},
...
thisUser = JSON.parse(Slocal.get('StaticUserData')) || {
	username: 'Object Hub',
	ID: 0,
	role: 0,
	isActive: 0,
...
if (token) 
	$.cfg.defaultHeaders['user-token'] = token;

let reStart = (drop = 0, errorId = 0)=>{
	let helperInit = ()=>{
		$.link.get();
		Fingerprint.generate()
			.then(fpData=>{
				fp = fpData;
				$.cfg.defaultHeaders['device-static'] = fp.staticName;
				//$.http.req('GET', `${nodeServer}loginT`)
				helperRequest(sData[2]+'loginT'+php)
					.then(data=>{
						...
						if (serverResp[0].token == 'false') {
							delete $.cfg.defaultHeaders['user-token'];
	...
	if (errorId !== 0) {
		if ($id('debug'+errorId))
			subWindows.close($id('debug'+errorId).parentElement.parentElement.id);
		$.err.errors[errorId] = $.err.errors[$.err.count];
		delete $.err.errors[$.err.count];
		$.err.count--;
		if ($.err.count === 0) {
			$id('errorBoxCount').remove();
		} else {
	...
	if (parseInt(Slocal.get('LangVer')) !== currentLangVer) {
		...
		$.langs.main = JSON.parse(applyLanguage(Slocal.get('Lang')));
		helperInit(drop);
	}
},
mainPageCache = {
...
rebootPageContent = ()=>{
	...
	refreshPageUrls.forEach(p=>{
		if ($.link.compileLink().includes(p))
			return $.link.get();
	})
	if ($id('profileWindow'))
		$.link.get();
...
reportError = (errorId)=>{
	let text = encodeURIComponent($id('debugMega'+errorId).innerHTML);
	Loading();
...
php = '.php',
nodeServer = 'http://127.0.0.1:3000/';
$.cfg.langAddr = `${scritpsUrl}/langs/`;
errInfo = 
	`LOCATION: ${location}\n`+
	`USERID: ${thisUser.ID}\n`;
let
// Главные html теги
	helperMain = $id('1st'),
	helperIcon = link = $q("link[rel~='icon']"),
	helperTitle = $q('title'),
// #endregion
// #region компоненты
...
contentRender = function(
	...
	let html = `<div class=framegdps ${joinData ? `id=news${preHtml.ID}` : ''} ${isComm == 0 ? 'style="width:calc(100% - 40px)"' : ''}>`+
		(preHtml.img || tags ?
		`<img onerror="console.warn('broken link');this.src='${helperUrl}imgs/hubbig.png'" align="left" src="${decodeURIComponent(preHtml.img)}" width=128px height=128px style="border-radius:calc(var(--def-border)*2)">`
		: '')+
...
contentRenderMinu = function(
	...
	`<div class="framegdps" styling="${btnFuncs[2]}${data.ID}" style="${preHtml[3]}">`+
		(renderImage ? `<div class=loh style="min-height:128px">` : '')+
			...
			`</p>` : '')+
			(renderImage ? `<img onerror="console.warn('broken link ${decodeURIComponent(data.img)}');this.src='${helperUrl}imgs/hubbig.png'" align="left" src="${decodeURIComponent(data.img)}" width=128px height=128px style="border-radius:calc(var(--def-border)*2)">`+
		`</div>` : '')+
		
		`<img class="${imgClass[0]}" id="guideimg" style=width:${banWidth}px;height:${banHeight}px src="${decodeURIComponent(data.ban)}" onerror="console.warn('broken link ${decodeURIComponent(data.ban)}');this.src='${helperUrl}imgs/hubemp.png'">`+
...
writeTag = (type,tag)=>{
	...
	if (!helperFindData[INDEX].includes(tag)) {
		$id(elemId+tag).setAttribute('class','tagSel');
		helperFindData[INDEX].push(tag);
	} else {
		$id(elemId+tag).setAttribute('class','tagUns');
...
setMethod = (Method)=>{
	$id('method'+helperFindData[0]).setAttribute('class','tagPre');
	helperFindData[0] = Method;
	$id('method'+helperFindData[0]).setAttribute('class','tagSel');
	sendFinder();
},
sendFinder = (page = 0, query = '')=>{
	...
	if (query === '') {
		...
		let enteredName = $id('gdpsNameInput').value;
		if (enteredName != '')
	...
	//$.http.req('POST', `${nodeServer}${sData[3].slice(39)}global?${query}&page=${page}&channel=${helperFindData[3]}`, `token=${token}&device=${fp.staticName}`)
		.then(data=>{
...
modifyFindTags = (channel)=>{
	if ($id(`tags${helperFindData[3]}`))
		$id(`tags${helperFindData[3]}`).style.display = 'none';
	let actsCount = $qa('[class=tagSel]').length - 1;
	$qa('[class=tagSel]').forEach(el=>{
	...
	helperFindData[1] = [];
	helperFindData[2] = [];
	$id('channel'+helperFindData[3]).setAttribute('class','tagPre');
	helperFindData[3] = channel;
	$id('channel'+helperFindData[3]).setAttribute('class','tagSel');
	if ($id(`tags${helperFindData[3]}`))
		$id(`tags${helperFindData[3]}`).style.display = '';
	sendFinder();
},
...
getFind = (channel, id, joinData = 0)=>{
	...
	helperRequest(`${sData[0]}camp${php}?id=${id}`)
		.then(data=>{
			...
			let serverResp = JSON.parse(data);
			$.link.set(smallString+'='+id, serverResp.gdps.title);
			...
			$id('imageBG').src = decodeURIComponent(serverResp.gdps.ban);

			if (isOwner)
				$id('news').insertAdjacentHTML('afterbegin', `<div class=framegdps style="width:calc(100% - 40px)">`+newsWindow(id,tinyStr)+`</div>`);
			$id('news').innerHTML = basicButton(getTrans('newsList'), `helperNews('${id}${newsChannel}',${isOwner})`)+
				RenderNews(serverResp.news,2,'getCamp');
			if (Object.keys(serverResp.news).length > 10)
				$id('news').insertAdjacentHTML('beforeend',insertBtn(`loadMoreNews(${id},'get${bigString}',1,2)`));
			
			$id('insertable').innerHTML = html;
			setImgSize();
			Loading(1);
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
	...
	helperRequest(`${sData[0]}news${php}?id=${gdpsId}&page=${page}`)
		.then(data=>{
			...
			if (data !== '{}') {
				...
				if ($id('news'))
					$id('news').insertAdjacentHTML('beforeend',html);
				else 
...
getNewsWithComments = (newsId, contentId = 0, backFuncPre = '', commBackFunc = '')=>{
	...
	helperRequest(`${sData[0]}newsC${php}?id=${newsId}`)
		.then(data=>{
			...
			let serverResp = JSON.parse(data);
			$.link.set('newsC='+newsId+'|'+contentId+'|'+backFuncPre, serverResp.gdps['n'+newsId].title);
...
loadGlobalVacs = (page)=>{
	if ($id('nextGdps'))
		$id('nextGdps').remove();
	Loading();
	helperRequest(`${sData[8]}newsAll${php}?page=${page}`)
		.then(data=>{
			Loading(1);
			if (data !== '{}') {
				let parsedData = JSON.parse(data),
...
getVacsWithComments = (vacId)=>{
	...
	helperRequest(`${sData[0]}vacsC${php}?id=${vacId}`)
		.then(data=>{
			...
			let serverResp = JSON.parse(data);
			$.link.set('VacsC='+vacId, serverResp.gdps['v'+vacId].title);
			let dataForNextButton = `${vacId},3,1`,
				html = '';
				
			html = renderVacancy(serverResp.gdps,thisUser.role,'f');

			innerComments(renderComms(serverResp.comments, 12, dataForNextButton), 0);
			$id('insertable').innerHTML = html;
			Loading(1);
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
...
getWikis = (page)=>{
	...
	Loading();
	//$.http.req('GET', `${nodeServer}${sData[7].slice(39)}getWikis?page=${page}`)
...
innerGdpsPlace = (textContent, insertType = 0, otherId = '')=>{
	...
	if (insertType == 0) // профили
		$id('GDPSesPlace'+otherId).innerHTML = textContent;
	else if (insertType == 512)
		$id('GDPSesPlace'+otherId).insertAdjacentHTML('beforebegin', textContent);
	else if (insertType == 511)
		$id('GDPSesPlace'+otherId).insertAdjacentHTML('afterbegin', textContent);
	else if (insertType >= 1) // в поиске устарел
		$id('GDPSesPlace'+otherId).insertAdjacentHTML('beforeend', textContent);
	else // в поиске но лучще это
		$id('GDPSesPlace'+otherId).insertAdjacentHTML('afterend', textContent);
},
...
innerComments = (textContent, insertType = 0)=>{
	...
	if (insertType == 0) // при рендере гдпса
		$id('comments').innerHTML = textContent;
	else 
		// а эт вроде когда "показать больше"
		$id('comments').insertAdjacentHTML('beforeend', textContent);
},
...
innerGuides = (textContent, insertType = 0)=>{
	...
	if (insertType == 0)
		$id('guidesPlace').insertAdjacentHTML('beforeend',textContent);
	else 
		$id('guidesPlace').insertAdjacentHTML('afterend',textContent);
},
// #endregion
// #region разные формы
sendRegisterForm = (wId)=>{
	let username = $id('LGusername').value,
		password = $id('LGpassword').value,
		email	= $id('LGemail'	 ).value,
			...
			.then(data=>{
				switch (data) {
					...
					default:
						...
						$id('regBtn').remove();
						$id('btnLogin').innerHTML = `<span style="position:absolute;right:0;top:-8px">${thisUser.username}</span>`;

						if ($id('regBtn2')) {
							$id('regBtn2').innerHTML = getTrans('logout', 0);
							$id('regBtn2').setAttribute('data-trans', 'logout');
							$id('regBtn2').setAttribute('onclick', 'gLogout()');
						}
						if ($id('btnLogin2')) {
							$id('btnLogin2').innerHTML = thisUser.username;
							$id('btnLogin2').removeAttribute('data-trans');
							$id('btnLogin2').setAttribute('onclick', 'profilePage()');
						}
						profilePage();
						$qa('[isloginwindow]').forEach(el=>{
							subWindows.close(el.id);
						});
				}
				Loading(1);
			})
...
sendLoginForm = (wId)=>{
	...
	if (!ignoreCap && !reCAP) {
		...
		//$.http.req('POST', `${nodeServer}${sData[5].slice(39)}login`, `username=${username}&password=${password}`+`&g-recaptcha-response=${reCAP}`+fp.urlDone)
			.then(data=>{
				switch (data) {
					case '-1':
						megaAlert('wrongPass');
...
sendLike = (id, channel, isComm = 0)=>{
	...
	helperRequest(`${sData[1]}like${php}`, data)
		.then(data=>{
			...
			if (isComm) 
				prefix = 'Comm';
			$id(likePlace + prefix + id).textContent = likeValue[0];
			$id(dislPlace + prefix + id).textContent = likeValue[1];
...
sendComm = (id, channel, likeChannel)=>{
	...
	helperRequest(`${sData[1]}comment${php}`, data)
		.then(data=>{
			...
			innerComments(renderComms(serverResp,likeChannel,dataForNextButton), 0);
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
},
editComm = (id, channel)=>{
	if ($id('commEdit'+id) !== null)
		return;
	subWindows.open('commEdit',
		`<textarea class=framelabel style=width:250px id=editText-C${id}>${$id('commText'+id).textContent}</textarea><br>`+
		basicButton(getTrans('commSend'), `modifyComm(${id},${channel})`)
	, 'commEdit'+id);
},
modifyComm = (id, channel)=>{
	let text = $id('editText-C'+id).value,
			data = `id=${id}&type=${channel}&text=${text}`;
	Loading();
	//$.http.req('POST', `${nodeServer}${sData[9].slice(39)}modify`, data)
	helperRequest(`${sData[1]}commentModify${php}`, data)
		.then(data=>{
			Loading(1);
			subWindows.close($q(`[commEdit${id}]`).id);
			if (data == '-4') {
				megaAlert('commSizeFail');
				return;
			}
			...
			if (data != '')
				if ($id('commText'+id))
					$id('commText'+id).textContent = data;
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
},
deleteComm = (id, channel)=>{
	Loading();
	//$.http.req('GET', `${nodeServer}${sData[9].slice(39)}remove?ide=${id}&type=${channel}`)
	helperRequest(`${sData[4]}comment${php}?ide=${id}&type=${channel}`)
		.then(data=>{
			if (data == '-1')
				return $.err.log('Access denied');
			$id('comm'+data).remove();
			Loading(1);
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
...
modifyNews = (id, gdpsId)=>{
	let title = $id('editNews1-N'+id).value,
			text = $id('editNews2-N'+id).value,
	...
	helperRequest(`${sData[1]}newsModify${php}`, data)
		.then(data=>{
			Loading(1);
			subWindows.close($q(`[newsEdit${id}]`).id);
			if (data == '-3') {
				megaAlert('newsNone');
				return;
			}
			...
			if (data != '')
				if ($id('news'+id)) {
					$id('Ntitle'+id).textContent = parsedData[0];
					$id('Ntext'+id).innerHTML = text;
				}
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
},
...
pageWikiList = ()=>{
	...
	innerMain(html);
	Loading();
	//$.http.req('GET', `${nodeServer}${sData[7].slice(39)}getWikis`)
...
globalVacs = ()=>{
	...
	innerMain(html);
	Loading();
	//$.http.req('GET', `${nodeServer}${sData[8].slice(39)}getVacs?page=0`)
...
helperNews = (gdpsId, renderOwnButton = 0)=>{
	...
	helperRequest(`${sData[0]}news${php}?id=${gdpsInt}`)
		.then(data=>{
			$.link.set('newsList='+gdpsId+'|'+renderOwnButton);
			Loading(1);
			if (data == '{}') {
				innerGdpsPlace(`<h1${getTrans('newsNone')}/h1>`, 1);
			} else {
...
setImgSize = ()=>{
	...
	if (window.innerWidth >= 700) { // large screen
		if ($id('imageBG')) {
			$id('imageBG').style = '';
			let alphaY = $id('imageBG').getBoundingClientRect().height - 2;
			$id('gdpsalpha').style = `z-index:-5;position:absolute;top:${alphaY}px`;
			return;
		}
	} else if($id('gdpsalpha')) { // small screen
		$id('gdpsalpha').style = `z-index:-5`;
		offset = -6;
	}
	if ($id('gdpsalpha') && $id('imageBG')) {
		let darkElement = $id('gdpsalpha').getBoundingClientRect(),
		imgposY = darkElement.y + offset,
		imgposX = imgposY * 2.4;

		
		$id('imageBG').style.width = imgposX+'px';
		$id('imageBG').style.height = imgposY+'px';
	}
},
BETA_fixImg = (url)=>{
	if (!renderBeta)
...
ADwrite = (userId = '')=>{
	...
	subWindows.open('writeAlarm', 
		`<h1>Write to support</h1>`+
		`<form onsubmit="return enterFormData(this,'writeAlarm${php}')">`+
			`<input name=windowId value=${subWindows.count} type=hidden>`+
			...
			`<button onclick="subWindows.close('${subWindows.count}writeAlarm')" class=loginbtn>close</button>`+
			`<input type=submit value=send class=loginbtn>`+
		`</form>`
	);
},
loginPage = ()=>{
	let id = subWindows.open('logonWindow',
		...
		`</button><br><br>`+
		`<div id=${subWindows.count}cap class=g-recaptcha data-sitekey=${helperCaptchaSiteKey}></div>`+
		`<button style=width:100% onclick="innerMain(dropWindow())" class="loginbtn"${getTrans('remindPass')}/button><br><br>`+
		`<button style=width:100% onclick="sendLoginForm(${subWindows.count})" class="loginbtn"${getTrans('joinToGdps')}/button><br>`+
...
registerPage = ()=>{
	let id = subWindows.open('logon2Window',
		...
		`<div id=${subWindows.count}cap class=g-recaptcha data-sitekey=${helperCaptchaSiteKey}></div>`+
		`<button style=width:100% onclick="sendRegisterForm(${subWindows.count})" class="loginbtn"${getTrans('register')}/button><br>`+
...
gdpsReport = (gdpsId)=>{
	subWindows.open('REPform',
		`<h1${getTrans('report01')}/h1>`+
		`<form id=${subWindows.count}formREP onsubmit="return enterFormData(this,'report${php}')">`+
			`<input type=hidden value="${subWindows.count}REPform" name=windowId>`+
			...
			`<button onclick="$id('${subWindows.count}formREP').setAttribute('onsubmit','return false');subWindows.close('${subWindows.count}REPform')" class=loginbtn${getTrans('otmena')}/button>`+
			`<input type=submit class=loginbtn${getTrans('commSend', 'inputValue')}`+
		`</form>`
	);
},
forumReport = (postId)=>{
	subWindows.open('REPform2', 
		`<h1${getTrans('report01')}/h1>`+
		`<form id=${subWindows.count}formREP onsubmit="return enterFormData(this,'report${php}')">`+
			`<input type=hidden value="${subWindows.count}REPform2" name=windowId>`+
			...
			`<button onclick="$id('${subWindows.count}formREP').setAttribute('onsubmit','return false');subWindows.close('${subWindows.count}REPform2')" class=loginbtn${getTrans('otmena')}/button>`+
			`<input type=submit class=loginbtn${getTrans('commSend', 'inputValue')}`+
		`</form>`
	);
},
getConfInfo = (step = 0)=>{
	if (step == 0) {
		subWindows.open('getLogin',
			`<form id=${subWindows.count}formLINK method=post onsubmit="return false">`+
				`<input class=framelabel id=LGpassword${getTrans('login02', 'input')}<br>`+
				`<button onclick="$id('${subWindows.count}formLINK').setAttribute('onsubmit','return false');subWindows.close('${subWindows.count}getLogin')" class=loginbtn${getTrans('otmena')}/button>`+
				`<button onclick=getConfInfo(1);subWindows.close('${subWindows.count}getLogin') class=loginbtn${getTrans('commSend')}/button>`+
			`</form>`
		);
	} else {
		let password = $id('LGpassword').value;
		Loading();
			...
			.then(data=>{
				if (data == '-1') {
					...
					let parsedData = JSON.parse(data),
						...
						`<button onclick="subWindows.close('${subWindows.count}getLogin2')" class=loginbtn${getTrans('back')}/button>`;			
						subWindows.open('getLogin2', html2);
				}
				Loading(1);
			})
			.catch(e=>{console.error(e);$.err.handlePromise(e)});;
...
// #endregion
// #region newHelper.js - свитчеры
makeSwticher = (switchType = 0, switcherElemId = '', switcherHTML = '', innerElementId = '', switcherName)=>{
	if (switchType === 0) 
		if (!$id(switcherElemId))
			if (typeof innerElementId === 'string') {
				if ($id(innerElementId))
					$id(innerElementId).insertAdjacentHTML('beforeend', switcherHTML);
			} else 
				innerElementId.forEach(el => el == 'object' ? el.insertAdjacentHTML('beforeend', switcherHTML) : '');
		else
			$id(switcherElemId).remove();
	else
		$id(switcherElemId).remove();
	if (false)
		if (!$id(switcherElemId)) // location.search.replace('?','').split('&').includes('switcher='+innerElementId)
...
switchAdd = (name)=>{
	$.link.add('switcher='+name);
	return false;
},
swtichRemove = (name)=>{
	$.link.remove('switcher='+name);
	return false;
},
...
switchLangMenu = ()=>{ // makeSwticher(0,'switchHtmlLang2', switchLangMenu(), 'switchHtmlLang', 'switchLangMenu')
	...
	langList.forEach(lang=>{
		preLang += 
		`<button onclick="replaceLang('${lang}');makeSwticher(1,'switchHtmlLang2')" style="width:40px;margin:2px" class="emptybtn">`+
...
RenderNews = (data, isComm = 0, backFunc = 'getCamp', commBackFunc = '')=>{
	...
	for (let ide in data)	{
		...
		`<div>${gdpsData.hasFile == '' ? '' : `<img class=newsImage src=${helperUrl}imgs/customnews/${gdpsData.ID}.${gdpsData.hasFile}>`}</div>`+
		`<div style="margin-top:15px">`+
			`<div class="likezone">`+
...
vacRespond = (vacId)=>{
	...
	helperRequest(`${sData[8]}apply${php}?id=${vacId}`)
		.then(data=>{
			Loading(1);
			megaAlert('reported');
			let applyBtn = $id('a'+vacId);
			if (applyBtn) {
				applyBtn.setAttribute('data-trans', 'vacResponded');
				applyBtn.setAttribute('onclick', `vacUnrespond(${vacId},${data})`);
				applyBtn.textContent = getTrans('vacResponded',0);
			}
		})
		.catch(e=>{console.error(e);$.err.handleRejection(e)});;
},
...
seePassword = ()=>{
	if ($id('LGpassword').type == 'password') {
		$id('LGpassword').type = 'text';
		$id('LGbtn').src = helperUrl+'imgs/PSsee.svg';
	} else {
		$id('LGpassword').type = 'password';
		$id('LGbtn').src = helperUrl+'imgs/PShide.svg';
	}
},

profileSwitcherPhone = ()=>{
	let
	profileNavPhone = $id('phoneSelectorSmall'),
	profileContent = $id('helperContentProfile');
...
switchMobileMain = ()=>{
	let 
	helperNavPhone = $id('helperSecond'),
	pageContent = $id('helperContent');
	...
	if (headerPhoneSwitcher !== 2) {
		...
		if ($id('phoneSelectorSmall') && $id('phoneSelectorSmall').style.display === 'grid' && headerPhoneSwitcher !== 0) {
			headerPhoneSwitcher = 1;
...
enterFormData = (form, sendPlace)=>{
	...
	Loading();
	//$.http.req('POST', sendPlace, params)
	...
	.then(data=>{
		...
		switch (sendPlace) {
			case 'forumPost'+php:
				...
				subWindows.close($q('[forumpost]').id);
				break;
			case 'newsPost'+php:
				...
				break;
			case 'writeAlarm'+php:
				subWindows.close(FORMDATA.get('windowId')+'writeAlarm');
				break;
			case 'report'+php:
				megaAlert('reported', 1000);
				subWindows.close(FORMDATA.get('windowId'));
				break;
			case 'newGuide'+php:
			...
			case 'vacsEdit'+php:
				...
				if ($id('profileWindow')) 
					...
					subWindows.close($q('[vaceditadm]')?.id);
					globalVacs();
				}
				break;
			default:
...
updateFileSize = (value)=>{
	$id('fileSize').setAttribute('value', value);
	$id('fileSizeInt').innerHTML = value;
},
...
otherProfileMini = (userId)=>{
	...
	.then(data=>{
		...
		$.link.set('profiles='+userId, userData.username);
...
otherFindsWindow = (channel, userId)=>{
	...
	$.link.set('prof'+bigString+'s='+userId);
	Loading();
...
otherWikisWindow = (userId)=>{
	...
	.then(data=>{
		...
		`<div id=helperContentProfile>`+
			...
			`<div align=left class=gdps-list-place style="height:calc(100vh - 450px);overflow:auto;justify-content:left">`+
				gdpses+
			`</div>`+
		`</div>`;
		innerProfile(html);
...
window.addEventListener('beforeunload', e=>{
	if (Object.keys(subWindows.subws).length)
		e.preventDefault();
});
...
doHotkeyAction = (keyCode)=>{
	const profileElementHas = !!$id('profileWindow'),
		keysList = {
			[keyActions.profile]: ()=>{
...
keyActions = JSON.parse(Slocal.get('KeysCfg')) || {
	main: 'ShiftLeft',
	profile: 'KeyQ',
	yourCamps: 'KeyC',
	yourShows: 'KeyS',
...
keyCustomActions = JSON.parse(Slocal.get('KeysCustomCfg')) || [
	'',
	'',
	'',
	'',
