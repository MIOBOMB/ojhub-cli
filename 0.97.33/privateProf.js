...
editNick = ()=>{
	...
	helperRequest(`${sData[5]}setNickname${php}?name=${newNick}`)
		.then(data=>{
			...
			$id('oldNick').innerHTML = data;
			$id('newNick').innerHTML = '';
		})
},
editResumePre = ()=>{
	$id('newResume').innerHTML =
...
editResume = ()=>{
	let newResume = $id('newResume2').value.replaceAll('\n','\\n');
	helperRequest(`${sData[5]}setResume${php}`, `name=${newResume}`)
		.then(data=>{
			thisUser.resume = data;
			if (data != '')
				$id('oldResume').innerHTML = `<span class=framegdpsOld style="display:block;width:calc(100% - 40px)">${data.replaceAll('\\n', '<br>')}</span>`;
			else 
				$id('oldResume').innerHTML = '';
			$id('newResume').innerHTML = '';
		})
},
editSocialsPre = ()=>{
	$id('newSocials').innerHTML =
...
editSocials = ()=>{
	let newSocials = $id('newSocials2').value.replaceAll('\n','\\'+'n');
	helperRequest(`${sData[5]}setSocials${php}`, `name=${newSocials}`)
		.then(data=>{
			thisUser.socials = data;
			if (data != '')
				$id('oldSocials').innerHTML = `<span class=framegdpsOld style="display:block;width:calc(100% - 40px)">${data.replaceAll('\\n', '<br>')}</span>`;
			else 
				$id('oldSocials').innerHTML = '';
			$id('newSocials').innerHTML = '';
		})
},
...
guideSettings = (guideId)=>{
	if ($id('guidTag'+guideId))
		return;
	let windowName = 'guidSettings',
			helperWindowId = subWindows.open(windowName,
		...
		`</div>`+
		basicButton(getTrans('WINDOW-close'), `subWindows.close('${subWindows.count+windowName}')`)+'<br>'
	);
	Loading();
	helperRequest(`${sData[7]}getGuide${php}?id=${guideId}&wiki=${globalWiki}`)
		.then(data=>{
			...
			$id('wikiName'+guideId).innerHTML = guideinfo[1];
			$id('guidTag'+guideId).value = guideinfo[3];
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
},
setWikiTag = (guideId)=>{
	let tagName = $id('guidTag'+guideId).value;
	Loading();
...
getFullAlarm = (id)=>{
	...
	.then(data=>{
		...
		`</div>`;
		if ($id('btn'+id))
			$id('btn'+id).style.backgroundColor = '';
		$id('alarms_big').innerHTML = html;
		Loading(1);
	})
	.catch(e=>{console.error(e);$.err.handlePromise(e)});;
},
...
JEedit = (gdpsId)=>{
	...
	helperRequest(`${sData[1]}setj${php}?id=${gdpsId}`)
		.then(data=>{
			...
			$id('JE'+gdpsId).innerHTML = getTrans(data, 0);
			Loading(1);
		});
},
ballsUp = (gdpsId, type = 'g')=>{
	...
	helperRequest(`${sData[1]}bump${php}?id=${gdpsId}`)
		.then(data=>{
			if (data == 'no') 
				return $id('BL'+gdpsId).innerHTML = getTrans('campunckecked', 0);
			...
			$id('BL'+gdpsId).innerHTML = canBump;
			Loading(1);
		});
},
coownersMenu = (id, contentType)=>{
	if ($q(`[coowners_${contentType}_${id}]`))
		return 0;
	Loading();
...
generateSocialNetworks = (HTMelement, editModeValue = '')=>{
	if ($qa(`[socialnetwork=${HTMelement.value}]`).length == 0)
...
addVacsPre = ()=>{
	...
	if (gdpsesPre.length == 0)
		return subWindows.open('vacsaddmodal',
		`<p${getTrans('addVacs03a')}/p>`+
		basicButton(getTrans('otmena'), "subWindows.close($q('[vacsaddmodal]').id)")
	, 'vacsaddmodal');
	subWindows.open('vacsaddmodal',
		`<p${getTrans('addVacs01a')}/p>`+
		`<select class="framelabel" onchange="subWindows.close($q('[vacsaddmodal]').id);addVacs(this.value.split(',')[0],this.value.split(',')[1])">`+
...
tryAddShow = ()=>{
	let links = document.getElementsByName('links[]').length,
			tags = $qa('[name="tags[]"]:checked').length,
			os = $qa('[name="os[]"]:checked').length;
	console.log(links);
	if (links === 0) {
...
createForum = (forumId, step = 0)=>{
	if (step == 0) {
		subWindows.open('forumAlert',
			`<form id=${subWindows.count}formLINK method=post onsubmit="return false">`+
				`<p style=min-width:180px${getTrans('forumAlert')}/p><br>`+
				`<button onclick=createForum(${forumId},1);subWindows.close('${subWindows.count}forumAlert') class=loginbtn${getTrans('yes')}/button>`+
				`<button onclick="$id('${subWindows.count}formLINK').setAttribute('onsubmit','return false');subWindows.close('${subWindows.count}forumAlert')" class=loginbtn${getTrans('no')}/button>`+
			`</form>`
		)
	} else {
		Loading();
...
removeVacPre = (id, gdpsId)=>{
	if ($q(`[vacremove${id}]`))
		return;
	subWindows.open('vacRemove',
		`<p${getTrans('removeSure')}/p>`+
		basicButton(getTrans('delete'), `removeVac(${id},${gdpsId})`)+
		basicButton(getTrans('otmena'), `subWindows.close($q(\`[vacremove${id}]\`)?.id)`)
	, 'vacremove'+id);
},
removeVac = (id, gdpsId)=>{
		Loading();
		helperRequest(`${sData[8]}removeVac${php}?id=${id}&gdpsId=${gdpsId}`)
			.then(data=>{
				if (data == 1) {
					$id('v'+id).remove();
					subWindows.close($q(`[vacremove${id}]`)?.id);
				}
				Loading(1);
			})
			.catch(e=>{console.error(e);$.err.handlePromise(e)});;
...
removeAplPre = (id, gdpsId)=>{
	if ($q(`[aplremove${id}]`))
		return;
	subWindows.open('aplRemove',
		`<p${getTrans('removeSure')}/p>`+
		basicButton(getTrans('delete'), `removeApl(${id},${gdpsId})`)+
		basicButton(getTrans('otmena'), `subWindows.close($q(\`[aplremove${id}]\`)?.id)`)
	, 'aplremove'+id);
},
removeApl = (id, gdpsId)=>{
		Loading();
		helperRequest(`${sData[8]}removeApl${php}?id=${id}&gdpsId=${gdpsId}`)
			.then(data=>{
				if (data == 1) {
					$id('a'+id).remove();
					subWindows.close($q(`[aplremove${id}]`)?.id);
				}
				Loading(1);
			})
			.catch(e=>{console.error(e);$.err.handlePromise(e)});;
...
connectContent = (wikiId, step = 0)=>{
	if (step == 0) {
		...
		subWindows.open('contentToWiki', 
			`<form id=${subWindows.count}formLINK method=post onsubmit="return false">`+
				// `<input class=framelabel id=contentId placeholder="ContentId"><br>`+
				`<select class=framelabel id=contentId>`+
					gdpses+
				`</select><br>`+
				`<button onclick="$id('${subWindows.count}formLINK').setAttribute('onsubmit','return false');subWindows.close('${subWindows.count}contentToWiki')" class=loginbtn${getTrans('otmena')}/button>`+
				`<button onclick=connectContent(${wikiId},1);subWindows.close('${subWindows.count}contentToWiki') class=loginbtn${getTrans('commSend')}/button>`+
			`</form>`
		)
	} else {
		let id = $id('contentId').value;
		Loading();
...
connectWiki = (contentId, type = 'c', step = 0)=>{
	...
	if (step == 0) {
		subWindows.open('wikiToContent',
			`<form id=${subWindows.count}formLINK method=post onsubmit="return false">`+
				// `<input class=framelabel id=wikiId placeholder="WikiId"><br>`+
				`<select class=framelabel id=wikiId>`+
					wikis+
				`</select><br>`+
				`<button onclick="$id('${subWindows.count}formLINK').setAttribute('onsubmit','return false');subWindows.close('${subWindows.count}wikiToContent')" class=loginbtn${getTrans('otmena')}/button>`+
				`<button onclick=connectWiki(${contentId},'${type}',1);subWindows.close('${subWindows.count}wikiToContent') class=loginbtn${getTrans('commSend')}/button>`+
			`</form>`
		)
	} else {
		let id = $id('wikiId').value;
		Loading();
...
setMainWiki = (contentId, guideId, step = 0)=>{
	if (step == 0) {
		if ($id('wikiId'))
			return;
		subWindows.open('setMainPage',
			`<form id=${subWindows.count}formLINK method=post onsubmit="return false">`+
				`<input class=framelabel id=wikiMainSet value=${guideId} placeholder="WikiId"><br>`+
				`<button onclick="$id('${subWindows.count}formLINK').setAttribute('onsubmit','return false');subWindows.close('${subWindows.count}setMainPage')" class=loginbtn${getTrans('otmena')}/button>`+
				`<button onclick=setMainWiki(${contentId},${guideId},1);subWindows.close('${subWindows.count}setMainPage') class=loginbtn${getTrans('commSend')}/button>`+
			`</form>`
		)
	} else {
		let guideId = $id('wikiMainSet').value;
		Loading();
...
removeDevice = (type, deviceId, isCurrent = '')=>{
	console.log(type, $q(`[device${deviceId}]`));
	if (type === 0) {
		if (!$q(`[device${deviceId}]`))
			return subWindows.open('removeDevice',
				`<p${getTrans('removeDevice'+isCurrent)}/p>`+
				basicButton(getTrans('yes'), `removeDevice(1,${deviceId},'${isCurrent}')`)+
				basicButton(getTrans('no'), `subWindows.close('${subWindows.count}removeDevice')`),
			` device${deviceId} `);
		else return false;
	}
	if (isCurrent == 'Current') {
		if ($q(`[device${deviceId}]`))
			subWindows.close($q(`[device${deviceId}]`).id);
		return gLogout();
	}
		helperRequest(`${sData[5]}removeDevice${php}`, `id=${deviceId}`)
			.then(()=>{
				Loading(1);
				$id('device'+deviceId).remove();
				if ($q(`[device${deviceId}]`))
					subWindows.close($q(`[device${deviceId}]`).id);
			})
			.catch(e=>{console.error(e);$.err.handlePromise(e)});;
...
setColorScheme = ()=>{
	Slocal.set('ColorScheme',$id('scheme').value);
	colorGenerator();
},
keyBindsList = ()=>{
	subWindows.open('listcommands',
		Markdown(
			`# Projects opening\n`+
	...
	if (isPhone == 0) 
		html = 
			`<div class=ANIM-swticherUp id=userSettings2>`+
				`<button class=loginbtn onclick="alarmsWindow();GetAlarms()" style=position:relative${getTrans('Alarms', 'textButton')}`+
		...
		html = 
			`<div class=ANIM-swticherUp id=userSettings2 style="display:grid;margin:0 8px 0 8px">`+
				`<button class=loginbtn onclick="alarmsWindow();profileSwitcherPhone();GetAlarms()" style=position:relative${getTrans('Alarms', 'textButton')}`+
				(thisUser.hasAlarms == 1 ? '<span style="position:absolute;top:-4px;right:-4px;border:solid red 5px;border-radius:var(--def-border-small)"></span>' : '')+
				`</button>`+
...
clrEditPage = ()=>{
	...
	sizeListeners.slice(1).split(',').forEach(id=>{
		...
		el.addEventListener('input', el=>{
			$id(el.target.name).innerHTML = el.target.value+'px';
		});
	});
...
keyBindsCfg = ()=>{
	...
	for (let actName in keyActions) {
		if ($id('key-'+actName))
			$id('key-'+actName).addEventListener('keydown', e=>{
  			e.preventDefault();
				$id('key-'+actName).value = e.code;//setAttribute('value', e.code);
				keyActions[$id('name-'+actName).textContent] = e.code;
			})
	}
	for (let i = 0; i < 10; i++) {
		if ($id('actkey-act'+i))
			$id('actkey-act'+i).addEventListener('input', e=>{
				let id = e.target.id.slice(e.target.id.length-1);
				keyCustomActions[id] = $id('actkey-act'+id).value;
			})
	}
};
...
editFind = (channel, gdpsId)=>{
	...
	//$.http.req('GET', `${nodeServer}${sData[10].slice(39)}edit?id=${gdpsId}`)
	.then (data=>{
		$.link.set('edit'+bigString+'='+gdpsId);
		let parsedData = JSON.parse(data),
			title = parsedData[0],
...
addVacs = (channel, gdpsId)=>{
	...
	$.link.set('add'+bigString+'='+channel+'|'+gdpsId);
	let html = 
	`<div id=helperContentProfile>`+
		`<h1${getTrans('add'+bigString)}/h1>`+
		`<form method=POST enctype="multipart/form-data" action='${smallString}Add${php}' onsubmit="return enterFormData(this,'${smallString}Add${php}')">`+
			`<label${getTrans('add'+bigString+'01')}/label><br><input class=framelabel type=text name=title style=width:100% required${getTrans(smallString+'Input01', 'input')}<br>`+
...
editVacs = (channel, gdpsId, vacId)=>{
	...
	helperRequest(`${sData[8]}edit${php}`, `id=${vacId}&gdpsId=${gdpsId}`)
		.then (data=>{
			if ($id('profileWindow')) 
				$.link.set('edit'+bigString+'='+channel+'|'+gdpsId+'|'+vacId);
			let parsedData = JSON.parse(data),
...
getVacancies = (channel, projId)=>{
	Loading();
	// helperRequest(false, '{"gdpsdata":["'+projId+'"],"vacs":{"v1":{"title":1,"tags":"[\\"1\\",\\"2\\"]","text":"loha","date":170002321,"gId":1,"gTitle":"lol","gChannel":0}}}')
	helperRequest(`${sData[8]}get${php}?id=${projId}`)
		.then(data=>{
			...
			`<div id=helperContentProfile>`+
				...
				`</div>`+
			`</div>`;
			$.link.set('vacans='+channel+'|'+projId);
			innerProfile(html);
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
};
	...
	helperRequest(`${sData[8]}applies${php}?id=${projId}&vacid=${vacId}`)
		.then(data=>{
			...
			`<div id=helperContentProfile>`+
				...
				`</div>`+
			`</div>`;
			$.link.set('applies='+channel+'|'+projId+'|'+vacId);
			innerProfile(html);
		})
		.catch(e=>{console.error(e);$.err.handlePromise(e)});;
};
...
GetAlarms = (page = 0)=>{
	...
	.then(data=>{
		if (data == '[]') {
			Loading(1);
			return $id('alarms_small').innerHTML = `<span${getTrans('newsNone')}/span>`;
		}
		let parsedData = JSON.parse(data),
			html = '',
			css = '';
		...
		$id('alarms_small').innerHTML = html;
		Loading(1);
	})
	.catch(e=>{console.error(e);$.err.handlePromise(e)});;
};
