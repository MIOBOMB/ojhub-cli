...
putAWord = (char = '')=>{
	...
	let elem = $id(`l${wordleLine}c${wordleColumn}`);
	elem.textContent = char;
	++wordleColumn;
	if (wordLenResult != wordleColumn)
		$id(`l${wordleLine}c${wordleColumn}`).textContent = '|';
	else 
		$q('[sendbtn]').style.background = '';
};
clearWord = ()=>{
	if (wordleColumn == 0 || wordleLine == 7)
		return;
	$q('[sendbtn]').style.background = 'var(--color-profile)';
	if (wordLenResult == wordleColumn) {
		--wordleColumn;
		$id(`l${wordleLine}c${wordleColumn}`).textContent = '|';
		return;
	...
	helperRequest(`${sData[2]}wordle${wordleLang}${php}`, `word=${wordPre.join('')}`)
		.then(data=>{
			...
			for (let i = 0; i < wordParsed.length; i++) {
				let char = $id(`l${wordleLine}c${i}`);
				if (wordParsed[i] == 2) {
					char.style.background = 'var(--color-light)';
					char.style.border = 'solid var(--color-light) 2px'
					$id(char.textContent).style.background = 'var(--color-light)';
				    $id(char.textContent).style.border = 'solid var(--color-light) 2px';
					continue;
				}
				if (wordParsed[i] == 1) {
					char.style.background = 'var(--color-window)';
					char.style.border = 'solid var(--color-window) 2px'
					$id(char.textContent).style.background = 'var(--color-window)';
				    $id(char.textContent).style.border = 'solid var(--color-window) 2px';
					continue;
				}
				$id(char.textContent).style.background = 'var(--color-bg)';
				$id(char.textContent).style.border = 'solid var(--color-white) 2px';
			}
