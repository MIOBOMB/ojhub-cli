...
setPage = (pageId)=>{
	$id('pagePlace').innerHTML = $id(pageId).innerHTML;
	...
	$id('pageNum').innerHTML = pageNxt;
	if ($id('p'+pagePre))
		$id('backPage').setAttribute('onclick', `setPage('p${pagePre}')`);
	if ($id('p'+pageNxt))
		$id('nextPage').setAttribute('onclick', `setPage('p${pageNxt}')`);
},
wikiText = (wikitext)=>{
	if (!wikitext) return '';
...
renderGuideMini = (parsedData, page = 0)=>{
	...
	for (let Id in parsedData) {
		...
		if (!Array.isArray(guid)) {
			...
			$id('wikiName').innerHTML = guid.title;
			continue;
		}
		Count++;
		if (Count == 5) {
...
falseGuideInWindow = (templateName)=>{
	let args = $qa('[argument]'),
			...
			content = $id(templateName+'-content').value.replaceAll('\n', '\\n');
	if (args.length !== 0) {
	...
	let subWindowId = subWindows.open('templateTest',
		`<div id=helperContent>`+
			`<h1 id=title${subWindows.count}></h1>`+
			`<div id=texts${subWindows.count}>${html}</div>`+
		`</div>`
...
getForumPost = (forumId, postId)=>{
	...
	helperRequest(`${sData[6]}getPost${php}?id=${postId}`)
		.then(data=>{
			...
			$.link.set('forumPost='+forumId+'.'+postId);
...
forumRenderMini = (forumId, parsedData, page = 0)=>{
	...
	for (let Id in parsedData) {
		...
		if (typeof guid !== 'object' && typeof guid === 'string') {
			$id('insertable').innerHTML = '<h1>'+guid+` ${basicButton('>+<', `uploadPost(${forumId})`)}</h1>`;
			continue;
		}
		Count++;
		if (Count == 5) {
...
uploadPost = (forumId)=>{
	...
	`<div id=helperContentProfile>`+
		...
		`</form>`+
	`</div>`;
	return subWindows.open('FORUMpost',
		html
	, 'forumpost');
};
...
pageGuides = (wiki, backButton = '')=>{
	...
	helperRequest(`${sData[7]}getWiki${php}?wiki=${wiki}`)
		.then(data=>{
			let parsedData = JSON.parse(data);
			$.link.set('wiki='+wiki, parsedData[0].title);
	...
	helperRequest(`${sData[7]}getGuide${php}?id=${id}&wiki=${wikiId}`)
		.then(data=>{
			...
			let parsedData = JSON.parse(data),
				...
				html = '';
			$.link.set('wikiPage='+id+'.'+wikiId, guideinfo[1]);
			...
			$id('title').innerHTML = guideinfo[1];
			if (guideinfo[3]) 
				$id('title').insertAdjacentHTML('afterend', guideinfo[2]);
			if (guideinfo[4]) 
